...
description: "Use for token discovery and token analytics: search tokens by name, symbol, or contract address; look up PEPE or other tokens; find trending tokens, top tokens, token rankings, token metadata, token info, token market cap, liquidity, volume, price-info, holder distribution, holders, and token research across XLayer, Solana, Ethereum, Base, BSC, Arbitrum, Polygon, and other chains. Trigger on find/search/look up token, what's trending, top tokens, holder analysis, who holds this token, token liquidity, or research a token. For simple price, K-line/chart, or public on-chain trade logs use opentrade-market. For personal holdings, orders, positions, or trade history use opentrade-cex first, then fall back to DEX/on-chain skills only if CEX has no relevant data. Do not trigger for a lone generic word like tokens or crypto, or for meme safety/rug/bundle/sniper checks."
license: Apache-2.0
metadata:
  author: 6551
  version: "1.0.2"
