...
pub enum TokenCommand {
    ...
    Search {
        ...
        query: String,
        /// Chains to search (comma-separated, e.g. "ethereum,solana" or "1,501")
...
pub async fn execute(ctx: &Context, cmd: TokenCommand) -> Result<()> {
    match cmd {
        ...
        TokenCommand::Toplist {
            chains,
            sort_by,
            time_frame,
        } => toplist(ctx, &chains, &sort_by, &time_frame).await,
...
async fn info(ctx: &Context, address: &str, chain: Option<String>) -> Result<()> {
    ...
    let data = client.post("/token/info", &body).await?;
    output::success(data);
    Ok(())
}
...
async fn price_info(ctx: &Context, address: &str, chain: Option<String>) -> Result<()> {
    ...
    let data = client.post("/token/price-info", &body).await?;
    output::success(data);
    Ok(())
}
