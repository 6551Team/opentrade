...
async fn gas(ctx: &Context, chain: &str) -> Result<()> {
    ...
    let client = ctx.client()?;
    let data = client
        .get("/gateway/gas", &[("chainIndex", chain_index.as_str())])
        .await?;
    output::success(data);
    Ok(())
}
...
async fn gas_limit(
    ...
    let result = client.post("/gateway/gas-limit", &body).await?;
...
async fn simulate(
    ...
    let result = client.post("/gateway/simulate", &body).await?;
    output::success(result);
    Ok(())
}
...
async fn broadcast(ctx: &Context, signed_tx: &str, address: &str, chain: &str) -> Result<()> {
    ...
    let result = client.post("/gateway/broadcast", &body).await?;
    output::success(result);
    Ok(())
}
...
async fn orders(ctx: &Context, address: &str, chain: &str, order_id: Option<&str>) -> Result<()> {
    ...
    let data = client.get("/gateway/orders", &query).await?;
    output::success(data);
    Ok(())
}
